...
case ${UID} in
  0)
    PROMPT="[%{${fg[blue]}%}%n@%m %c%{${reset_color}%}] %{${fg[blue]}%}$%{${reset_color}%} "
    PROMPT2="%B%{${fg[blue]}%}%_#%{${reset_color}%}%b "
    SPROMPT="%B%{${fg[blue]}%}%r is correct? [n,y,a,e]:%{${reset_color}%}%b "
    ;;
  *)
    PROMPT="[%n@%m %c] %{${fg[blue]}%}$%{${reset_color}%} "
    PROMPT2="%B%{${fg[blue]}%}%_$%{${reset_color}%}%b "
    SPROMPT="%B%{${fg[blue]}%}%r is correct? [n,y,a,e]:%{${reset_color}%}%b "
    ;;
  esac
...
RPROMPT='${time}%{$reset_color%}$(git_prompt_info)%{$reset_color%}$(git_prompt_status)%{$reset_color%}$(git_prompt_stash_count)%{$reset_color%}'
...
function git_prompt_stash_count(){
  ...
  if [ "$COUNT" -gt 0 ]; then
    echo "$ZSH_THEME_GIT_PROMPT_STASH_COUNT_PREFIX$COUNT"
  fi
}
